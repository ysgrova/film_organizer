    ...
    return (
      <div className="favorites">
        <input  value={this.state.title} onChange={this.handleInput}
              disabled={this.state.inputActive ? null : "disabled"}  className="favorites__name" placeholder="Enter the name of list" />
        ...
        <button type="button"    disabled={this.state.title==='' ? "disabled" : null }  onClick={() => this.handleSaveList()}
                className={`${this.state.title===''? "disabled-btn": "favorites__save"}  ${this.state.linkActive ? "link__none" : null}`}>
          Save
        </button>
        {console.log(this.state.title)}
        <a href={`http://localhost:3000/list/${this.state.textLink}`}
           className={`link__none ${this.state.linkActive ? "link__block" : null}`}
           target="_blank" >Share link</a>
      </div>
    );
  }
}
